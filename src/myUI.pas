...
begin
  //M�G NAGYOBB SEMMI (ez�rt "virtual")
end;
...
begin
  ...
  D3DXGetImageInfoFromFile(PChar('data/textures/gui/splash/' + splashnev), splashinfo); //felt�telezz�k, hogy egyform�k a splashek
...
begin
  if mit = VK_ESCAPE then
    case lap of
      0:lap:=3;
      1, 2:lap:=0;
      4, 5, 6, 7, 8:lap:=2; //TODO ezt friss�teni kell mindig
...
begin
  ...
  if (alap<(MENULAP_MAX - 1)) or (length(tegs[alap]) = 0) then //TAB vagy label �s �res
